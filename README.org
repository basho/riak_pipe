...
a Riak vnode partition index.  This is how parallelism is controlled
in =riak_pipe=: the "same" function will be run on multiple vnodes to
handle multiple inputs concurrently, if the partition decision
function evaluates to a different partition for each input.
...
until the worker catches us.  This backpressure prevents processes and
...
as a means of providing backpressure for workers that would otherwise
overwhelm their downstream partners.  When a queue is already at its
limit, the acknowledgement of reception of the input will be withheld
...
#fitting_spec {
   ...
   partfun = fun(_) -> 0 end, %% arity-1 function() | 'follow'
...
Finally, there would be only one worker spawned for the entire
cluster, on the "0" partition, since the =partfun= partition chooser
always chooses the "0" partition, regardless of input.

Using the value =follow= in the partfun field means inputs for the
...
the pipeline as a whole.  Currently supported options are:
 ...
 - sink :: =fitting()= | *=undefined=*
           ...
           this =undefined= to have the finall fitting send outputs as
...
The =queue_work/2= function evaluates the =partfun= function (from the
...
the =eoi= (End Of Inputs) to the following fitting, and the finall
...
The =recieve_result/1= function is also exported from =riak_pipe= to
...
evaluate the =partfun= function for the next fitting against the
...
transfered to the new node.  The worker process terminates soon after
=archive/1= completes.
...
example of how =archive/1= and =handoff/2= can be impelemented.
...
are maintained (and the function is evaluted) per-key.
...
reduce results locally, before an idential reduce with a consistent
...
The =riak_kv_pipe_get:partfun/1= function should always be used with
