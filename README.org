...
set: =log= and =trace=.  The =log= option should be set to =sasl= to
put these errors in the SASL log, or =sink= to have them delivered to
the sink.  The =trace= option should be set to at least =[error]=,
though =all= will also work.
